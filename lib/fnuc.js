...
(function() {
  var ARITY, CHAINABLE, I, add, all, and_, any, arity, binary, builtin, chainable, clone, compose, concat, contains, curry, div, each, eq, eql, evolve, exports, expose, filter, flip, fold, fold1, foldr, foldr1, get, gt, gte, has, head, ident, index, isplain, join, keys, last, lcase, lt, lte, map, match, max, merge, min, minifyNot, mixin, mod, mul, ncurry, not_, ofilter, omap, or_, partial, partialr, pick, replace, reverse, search, sequence, set, shallow, sort, split, sub, tail, tap, ternary, trim, type, typeis, ucase, unary, uncurry, uniq, values, _, _fold, _foldr, _toString,
    __slice = [].slice;
  ...
  CHAINABLE = split('clone shallow flip tap has get set keys values concat head tail last fold fold1 foldr foldr1 each map filter all any join reverse sort index contains uniq split match replace search trim ucase lcase add sub mul div mod min max gt gte lt lte eq and or not eql', ' ');
