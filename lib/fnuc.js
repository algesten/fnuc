...
(function() {
  var ARITY, CHAINABLE, I, Tuple, _, _fold, _foldr, _toString, aand, add, all, any, arity, binary, builtin, chainable, clone, compose, concat, contains, curry, div, drop, each, eq, eql, evolve, exports, expose, filter, flip, fold, fold1, foldr, foldr1, fst, get, groupby, gt, gte, has, head, ident, index, isplain, join, keys, last, lcase, len, lt, lte, map, match, max, merge, min, minifyNot, mixin, mod, mul, ncurry, nnot, nth, ofilter, omap, oor, partial, partialr, pick, replace, reverse, search, sequence, set, shallow, slice, snd, sort, split, sub, tail, take, tap, ternary, trim, tuple, type, typeis, ucase, unary, uncurry, uniq, unpack, unzip, values, zip, zipwith,
    slice1 = [].slice;
  ...
  Tuple = (function() {
    function Tuple(as) {
      ...
      for (i = q = 0, ref = as.length; q < ref; i = q += 1) {
  ...
  unzip = function(z) {
    ...
    r = (function() {
      var q, ref, results;
      results = [];
      for (n = q = 0, ref = l; q < ref; n = q += 1) {
        results.push(new Array(z.length));
      }
      return results;
    })();
    un = function(i) {
    ...
    for (i = q = 0, ref = z.length; q < ref; i = q += 1) {
      z[i].unpack(un(i));
    }
