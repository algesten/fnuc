...
(function() {
  var ARITY, CHAINABLE, I, add, all, and_, any, arity, binary, builtin, chainable, clone, compose, concat, contains, curry, div, each, eq, eql, evolve, exports, expose, filter, flip, fold, fold1, foldr, foldr1, get, groupby, gt, gte, has, head, ident, index, isplain, join, keys, last, lcase, lt, lte, map, match, max, merge, min, minifyNot, mixin, mod, mul, ncurry, not_, ofilter, omap, or_, partial, partialr, pick, replace, reverse, search, sequence, set, shallow, slice, slicefr, sliceto, sort, split, sub, tail, tap, ternary, trim, type, typeis, ucase, unary, uncurry, uniq, values, _, _fold, _foldr, _toString,
    __slice = [].slice;
  ...
  compose = function() {
    ...
    return ncurry(arity(last(fs)), fold1(fs, function(f, g) {
      return function() {
        var as;
        as = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
  ...
  slicefr = curry(function(s, n) {
    return s.slice(n);
  });

  sliceto = curry(function(s, n) {
    return s.slice(0, n);
  });
  ...
  eql = (function() {
    ...
    return curry(function(a, b) {
      ...
      return (and_(eqtype, (t = type(a)) === 'object' ? and_(eqplain, eqobj) : t === 'array' ? eqarr : function() {
        return false;
      }))(a, b);
    });
  })();
  ...
  CHAINABLE = split('clone shallow flip tap has get set keys values concat head tail last fold fold1 foldr foldr1 each map filter all any join reverse sort index contains uniq split match replace search slice, slicefr, sliceto, trim ucase lcase add sub mul div mod min max gt gte lt lte eq and or not eql groupby', ' ');
