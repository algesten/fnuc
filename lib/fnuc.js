...
(function() {
  var ARITY, CHAINABLE, I, add, all, and_, any, arity, binary, builtin, chainable, clone, compose, concat, contains, curry, div, each, eq, evolve, exports, filter, flip, fold, fold1, foldr, foldr1, get, gt, gte, has, head, ident, index, isplain, join, keys, last, lcase, lpartial, lt, lte, map, match, max, merge, min, minifyNot, mixin, mod, mul, ncurry, not_, ofilter, omap, or_, pick, replace, reverse, rpartial, search, sequence, set, shallow, sort, split, sub, tail, tap, ternary, trim, type, typeis, ucase, unary, uncurry, uniq, values, _, _toString,
    __slice = [].slice;
  ...
  ncurry = function(n, f, as) {
    ...
    nf = arity(l)(function() {
      ...
      cs = (bs.length <= l ? bs : bs.slice(0, l)).concat(as);
      if (cs.length < n) {
        return ncurry(n, f, cs);
      } else {
        return f.apply(null, cs);
      }
    });
    nf._curry = function() {
      return rpartial.apply(null, [f].concat(__slice.call(as)));
    };
    return nf;
  };
  ...
  flip = function(f) {
    ...
    return merge(rewrap(arity(arity(f))(function() {
      var as;
      as = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
  ...
  compose = function() {
    ...
    return ncurry(arity(last(fs)), fs.reduce(function(f, g) {
      return function() {
        var as;
        as = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
  ...
  filter = curry(binary(builtin(Array.prototype.filter)));

  fold = curry(function(as, f, v) {
    return as.reduce((function(p, c) {
      return f(p, c);
    }), v);
  });
  ...
  foldr = curry(function(as, f, v) {
    return as.reduceRight((function(p, c) {
      return f(p, c);
    }), v);
  });
  ...
  index = curry(binary(builtin(Array.prototype.indexOf)));
  ...
  and_ = curry(binary(function() {
    ...
    return function() {
      ...
      bs = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      return as.reduce((function(a, b) {
        return a && b.apply(null, bs);
      }), true);
    };
  }));

  or_ = curry(binary(function() {
    var as;
    as = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    return function() {
      var bs;
  ...
  CHAINABLE = ['clone', 'shallow', 'flip', 'tap', 'has', 'get', 'set', 'keys', 'values', 'concat', 'head', 'tail', 'last', 'fold', 'fold1', 'foldr', 'foldr1', 'each', 'map', 'filter', 'all', 'any', 'join', 'reverse', 'sort', 'index', 'contains', 'uniq', 'split', 'match', 'replace', 'search', 'trim', 'ucase', 'lcase', 'add', 'sub', 'mul', 'div', 'mod', 'min', 'max', 'gt', 'gte', 'lt', 'lte', 'eq', 'and', 'or', 'not'];
  ...
  exports.installTo = function(obj, force) {
    var tutti;
    if (!force) {
      if (obj.__fnuc) {
        return exports;
