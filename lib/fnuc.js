...
(function() {
  var ARITY, CHAINABLE, I, Tuple, aand, add, all, any, arity, binary, builtin, chainable, clone, compose, concat, contains, curry, div, drop, each, eq, eql, evolve, exports, expose, filter, flip, fold, fold1, foldr, foldr1, fst, get, groupby, gt, gte, has, head, ident, index, indexfn, isplain, join, keys, last, lcase, len, lt, lte, map, match, max, merge, min, minifyNot, mixin, mod, mul, ncurry, nnot, nth, ofilter, omap, oor, partial, partialr, pick, replace, reverse, search, sequence, set, shallow, slice, snd, sort, split, sub, tail, take, tap, ternary, trim, tuple, type, typeis, ucase, unary, uncurry, uniq, unpack, unzip, values, zip, zipwith, _, _fold, _foldr, _toString,
    __slice = [].slice;
  ...
  CHAINABLE = split('clone shallow flip tap has get set keys values concat head tail last fold fold1 foldr foldr1 each map filter all any join reverse sort index indexfn contains uniq split match replace search slice, drop, take, trim ucase lcase add sub mul div mod min max gt gte lt lte eq and or not eql groupby', ' ');
